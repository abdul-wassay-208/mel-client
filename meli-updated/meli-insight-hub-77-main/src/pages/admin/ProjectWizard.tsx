...
// Import hierarchy from central config
import {
  STRATEGIC_OBJECTIVES_DATA,
  OUTCOMES_DATA,
  INDICATORS_DATA,
  getOutcomesForObjective,
  getIndicatorsForOutcome,
...
export default function ProjectWizard() {
  ...
  const projectLeads = mockUsers.filter(u => u.role === 'project_lead');
  ...
  const getOutcomesForSO = (soName: string) => {
    return getOutcomesForObjective(soName);
  };

  const getFilteredIndicators = (outcomeId: string) => {
    return getIndicatorsForOutcome(outcomeId);
  };
